...
//  CS32HW3
//
//  Created by Nevin Liang on 8/8/20.
...
bool compareSensorPtr(const Sensor* lhs, const Sensor* rhs)
{
    ...
    // instead of rewriting the compareSensor function except with ptrs, we can just
    // call the previous compareSensor function passing in the ptr's values
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    // loop through the outermost vector s, from the 2nd element to the last
    for (int i = 1; i < s.size(); i++) {
        // nextitem is the current element
        Sensor nextItem = s[i];
        // loop from the current element to the first element, replacing if comp is true
        int loc = i;
        while (loc > 0 && comp(nextItem, s[loc - 1])) {
            s[loc] = s[loc - 1];
            loc--;
        }
        // place or "insert" the element here.
        s[loc] = nextItem;
    }
}
      ...
      // destruction time.
    {
     vector<Sensor> auxSensors(sensors);
      ...
      //        to point to the corresponding Sensor in auxSensors.
        ...
        // creates vector of sensor pointers by copying the stuff from auxSensors to the ptrs vector
      ...
      //        with compareSensorPtr as the ordering relationship.
        // sort using stl function and compareSensorPtr that we coded
      ...
      //        in sensors with the Sensors from auxSensors in the correct order.
        // copy back to sensors dereferencing pointers
        for (int i = 0; i < auxSensorPtrs.size(); i++)
